...
function drawWrappedTextWithPaging(params: {
  pdfDoc: PDFDocument;
  page: any;
  text: string;
  x: number;
  ...
  const { pdfDoc, text, x, y, maxWidth, lineHeight, font, fontSize, marginBottom, pageWidth, pageHeight } = params;

  let page = params.page;
  let cursorY = y;

  const pushLine = (line: string) => {
    if (cursorY < marginBottom + lineHeight) {
      page = pdfDoc.addPage([pageWidth, pageHeight]);
      cursorY = pageHeight - marginBottom; // on repart en haut (marge gérée par caller)
    }
...
export async function GET(req: Request) {
  try {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      const r = drawWrappedTextWithPaging({
      pdfDoc,
      page,
      text,
      x: margin,
...
for (let i = 0; i < pages.length; i++) {
  // Option: ne pas numéroter la couverture (i === 0). Sinon enlève la condition.
  if (i === 0) continue;
  drawPageNumber(pages[i], i, pages.length, A4_WIDTH, margin, font);
}
